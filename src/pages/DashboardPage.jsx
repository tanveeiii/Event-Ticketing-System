...
const DashboardPage = () => {
  ...
  const upcomingTickets = ticketsWithEventData.filter(ticket => 
    ticket.event && new Date(ticket.event.date) >= currentDate
  );
  const pastTickets = ticketsWithEventData.filter(ticket => 
    ticket.event && new Date(ticket.event.date) < currentDate
  );
      ...
      {activeTab === 'activity' && (
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          {tickets.length > 0 ? (
                    ...
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          Ticket purchased on {new Date(ticket.purchaseDate).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center">
            ...
            <p className="text-gray-600 text-center py-6">No activity to display yet.</p>
          )}
        </div>
      )}
    </div>
